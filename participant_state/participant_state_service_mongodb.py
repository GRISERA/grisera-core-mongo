...
class ParticipantStateServiceMongoDB(ParticipantStateService, GenericMongoServiceMixin):
    ...
    def save_participant_state(self, participant_state: ParticipantStateIn, dataset_name: str):
        ...
        """
        is_correct, error = self._check_related_fields(participant_state, dataset_name)
        if not is_correct:
            return error

        basic_ps = self.participant_service.add_participant_state(participant_state, dataset_name)
    ...
    def get_participant_states(self, dataset_name: str):
        ...
        """
        participant_state_dicts = self.get_multiple(dataset_name)
        results = [
    ...
    def get_single_dict(
        ...
        self._add_related_documents(
            participant_state_dict, dataset_name, depth, source, related_participant
        )
    ...
    def get_participant_state(
        ...
        Get single participation state object.
        """
        return self.get_single(id, dataset_name, depth, source, *args, **kwargs)
    ...
    def delete_participant_state(self, participant_state_id: Union[int, str], dataset_name: str):
        ...
        Returns:
            Result of request as participant state object
        """
        participant_state = self.get_participant_state(participant_state_id, dataset_name)
        if type(participant_state) is NotFoundByIdModel:
            return NotFoundByIdModel(
                id=participant_state_id,
                errors={"errors": "participant state not found"},
            )
        return self.participant_service.remove_participant_state(participant_state, dataset_name)
    ...
    def update_participant_state(
        ...
        Returns:
            Result of request as participant state object
        """
        existing_participant_state = self.get_participant_state(participant_state_id, dataset_name)
        ...
        return self.participant_service.update_participant_state(
            participant_state_id, existing_participant_state.dict(), dataset_name
        )
    ...
    def _check_related_fields(self, participant: ParticipantStateRelationIn, dataset_name: str):
        if not self._check_related_appearances(participant.appearance_ids, dataset_name):
            return False, ParticipantStateOut(
                errors={"errors": "one of given appearances does not exist"}
            )

        if not self._check_related_personalities(participant.personality_ids, dataset_name):
            return False, ParticipantStateOut(
                errors={"errors": "one of given personalities does not exist"}
            )

        if not self._check_related_participant(participant.participant_id, dataset_name):
            return False, ParticipantStateOut(
    ...
    def _check_related_appearances(self, appearance_ids: List[Union[str, int]], dataset_name: str):
    ...
    def _add_related_documents(
        ...
        if depth > 0:
            self._add_related_personalities(participant_state, dataset_name, depth, source)
            self._add_related_appearances(participant_state, dataset_name, depth, source)
#            self._add_related_participations(participant_state, dataset_name, depth, source)
            self._add_related_participant(participant_state, dataset_name, depth, source, participant)

    def _add_related_participant(
        self, participant_state: dict, dataset_name: str, depth: int, source: str, participant: dict
    ):
    ...
    def _add_related_participations(
        self, participant_state: dict, dataset_name: str, depth: int, source: str
    ):
