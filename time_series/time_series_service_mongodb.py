...
class TimeSeriesServiceMongoDB(TimeSeriesService):
    ...
    def save_time_series(self, time_series: TimeSeriesIn, dataset_name: str):
        ...
        if (
            ...
            and not self._check_related_observable_informations(
                time_series.observable_information_ids, dataset_name
            )
        ):
        ...
        related_measure = self.measure_service.get_measure(time_series.measure_id, dataset_name)
        ...
        return self.get_time_series(created_ts_id, dataset_name)

    def get_multiple(
        self, dataset_name: str, query: dict = {}, depth: int = 0, source: str = "", query_params=None
    ):
        results_dict = self.mongo_api_service.get_many_time_series(dataset_name, query, query_params)
    ...
    def get_time_series_nodes(self, dataset_name: str, params: QueryParams = None):
        ...
        """
        time_series_dicts = self.get_multiple(query_params=params, dataset_name=dataset_name)
    ...
    def get_time_series(
        ...
        self._add_related_documents(time_series, dataset_name, depth, source)
        if type(time_series) == NotFoundByIdModel:
            return time_series
        return TimeSeriesOut(**time_series)

    def get_time_series_multidimensional(self, time_series_ids: List[Union[int, str]], dataset_name: str):
        """
        Send request to graph api to get given time series
        Args:
    ...
    def delete_time_series(self, time_series_id: Union[int, str], dataset_name: str):
        ...
        self.mongo_api_service.delete_time_series(time_series_id, dataset_name)
        return get_response

    def update_time_series(
        self, time_series_id: Union[int, str], time_series: TimeSeriesPropertyIn, dataset_name: str
    ):
        """
        ...
        self.mongo_api_service.update_time_series_metadata(update_dict, time_series_id, dataset_name)
        return self.get_time_series(time_series_id, dataset_name)

    def update_time_series_relationships(
        self, time_series_id: Union[int, str], time_series: TimeSeriesRelationIn, dataset_name: str
    ):
        """
        Send request to graph api to update given time series
        ...
        self.mongo_api_service.update_time_series_metadata(
            time_series.dict(), time_series_id, dataset_name
        )
        return self.get_time_series(time_series_id, dataset_name)

    def transform_time_series(
        self, time_series_transformation: TimeSeriesTransformationIn, dataset_name: str
    ):
        """
    ...
    def get_time_series_for_observable_information(
        ...
        return self.get_multiple(dataset_name, query, depth, source)

    def _add_related_documents(self, time_series: dict, dataset_name: str, depth: int, source: str):
        if depth > 0:
            self._add_measure(time_series, dataset_name, depth, source)
            self._add_observable_informations(time_series, dataset_name, depth, source)

    def _add_measure(self, time_series: dict, dataset_name: str, depth: int, source: str):
    ...
    def _add_observable_informations(self, time_series: dict, dataset_name: str, depth: int, source: str):
    ...
    def _check_related_observable_informations(self, observable_information_ids: List, dataset_name: str):
