...
class ChannelServiceMongoDB(ChannelService, GenericMongoServiceMixin):
    ...
    def save_channel(self, channel: ChannelIn, dataset_name: str):
        ...
        """
        return self.create(channel, dataset_name)
    ...
    def get_channels(self, dataset_name: str):
        ...
        """
        results_dict = self.get_multiple(dataset_name)
        results = [BasicChannelOut(**result) for result in results_dict]
        return ChannelsOut(channels=results)

    def get_channel(
            self, channel_id: Union[str, int], dataset_name: str, depth: int = 0, source: str = ""
    ):
        """
    ...
    """
        return self.get_single(channel_id, dataset_name, depth, source)

    def _add_related_documents(self, channel: dict, dataset_name: str, depth: int, source: str):
